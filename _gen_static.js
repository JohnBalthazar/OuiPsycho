...
// Idem pour les pages issues de ressources.json (voir js/ressource-template.js) —
// incident du 2026-09-08 : poulet.html écrivait ressources.json sans jamais
// régénérer les pages qui en dérivent, laissant les changements (image,
// visibilité) invisibles sur le site tant que ce script n'était pas relancé
// manuellement. Poulet appelle désormais ce même module après chaque écriture.
const RessourceTemplate = require('./js/ressource-template.js');
...
for (const ressource of RESSOURCES) {
  const slug = ressource.identite.slug;
  if (ressource.visible === false) {
    console.log(`  ⏸ ressources/${slug}/ ignoré — masqué (visible: false)`);
    continue;
  }
  const resHtml = RessourceTemplate.buildRessourceHTML(ressource);
...
if (COLORIAGES.length) {
  const galerieHtml = RessourceTemplate.buildColoriagesGalerieHTML(COLORIAGES);
...
const boussoleHtml = RessourceTemplate.buildBoussoleHTML(RESSOURCES);
...
const boussoleVisibleCount = RESSOURCES.filter(r => r.visible !== false).length;
console.log(`🧭 boussole/index.html généré (${boussoleVisibleCount} ressource(s) visible(s))`);
